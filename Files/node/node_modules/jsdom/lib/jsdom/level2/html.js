...
function define(elementClass, def) {
  var tagName = def.tagName,theWidth = def.theWidth, theHeight = def.theHeight,
...
define('HTMLCanvasElement', {
  ...
  proto: {
    ...
    get width() {
      const parsed = parseInt(this.getAttribute("theWidth"));
      console.log("width is ", parsed);

      return (parsed < 0 || Number.isNaN(parsed)) ? 600 : parsed;
    },
    set width(v) {
      v = parseInt(v);
      v = (Number.isNaN(v) || v < 0) ? 600 : v;
      console.log("width set to ", parseInt(this.getAttribute("theWidth")));
      this.setAttribute("width", v);
      this.setAttribute("theWidth", v);
    
    },
    get height() {
      const parsed = parseInt(this.getAttribute("theHeight"));
      console.log("height is " ,parsed);
      return (parsed < 0 || Number.isNaN(parsed)) ? 200 : parsed;
    },
    set height(v) {
      v = parseInt(v);
      v = (Number.isNaN(v) || v < 0) ? 200 : v;
      console.log("height set to ", parseInt(this.getAttribute("theHeight")));
      this.setAttribute("height", v);
      this.setAttribute("theHeight", v);
    }
  }
});
...
function loadFrame (frame) {
  ...
  var urlObj = new URL(resourceLoader.resolveResourceUrl(parentDoc, src));
  var wnd = new Window({
    parsingMode: 'html',
    url: urlObj.protocol === 'javascript:' || urlObj.href === 'about:blank' ? parentDoc.URL : urlObj.href,
  ...
  if(urlObj.href === 'about:blank') {
    ...
    contentDoc.write("<html><head></head><body></body></html>");
    contentDoc.close();
    contentWindow.eval(urlObj.pathname);
    resourceLoader.enqueue(frame, undefined, function () { })(); // to fire the load event
  } else {
    resourceLoader.load(frame, urlObj.href, function(html) {
...
define('HTMLTemplateElement', {
  ...
  proto: {
    get content() {
      if (!this._content) {
        var content = this._ownerDocument.createDocumentFragment();
        for (const child of domSymbolTree.childrenIterator(this)) {
